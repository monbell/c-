...
private:
    ...
    int    m_row;
    int    m_col;
    bool   attacked; // to check whether a zombie object has been attcked before
};
    ...
    for (r = 0; r < rows(); r++)
    {
        for (c = 0; c < cols(); c++)
        {
            if(numZombiesAt(r+1, c+1) == 1)
                grid[r][c] = 'Z';
            else if(numZombiesAt(r+1, c+1) >= 2 && numZombiesAt(r+1, c+1) <=8)
            {
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    for(int i = 0; i < m_nZombies; i++) {
        if(m_zombies[i]->row() == r && m_zombies[i]->col() == c)
        {
            if(m_zombies[i]->getAttacked(dir) == true) // if m_zombies[i] is destroyed
            {
                delete m_zombies[i];
            ...
            else // if m_zombies[i] is attacked for the first time
                return false;
        }
    }
...
int main()
{
    doBasicTests(); // Remove this line after completing test. DELETE
    return 0;       // Remove this line after completing test. DELETE
    // Create a game
